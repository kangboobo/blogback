...
def compress_photo():
    '''调用压缩图片的函数
    '''
    src_dir, des_dir = "paintings/", "min_paintings/"
...
def handle_photo():
    ...
    -----------
    最后将data.json文件存到博客的source/paintings文件夹下
    '''
    src_dir, des_dir = "paintings/", "min_paintings/"
    ...
    with open("../source/paintings/ins.json", "w") as fp:
...
def git_operation():
    ...
    os.system('git commit -m "add paintings"')
    os.system('git push origin master')

if __name__ == "__main__":
    cut_photo()        # 裁剪图片，裁剪成正方形，去中间部分
    compress_photo()   # 压缩图片，并保存到mini_paintings文件夹下
